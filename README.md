...
### Процесс работы
1. Пользователь нажимает кнопку "Жалоба" или "Отзыв"
2. Администратор также видит кнопку "Статистика"
3. Для жалобы/отзыва: бот просит написать текст сообщения
4. Пользователь отправляет текстовое сообщение
5. Бот сохраняет данные в базу и отправляет email
6. Пользователь получает подтверждение с кнопками "Новое обращение" и "Помощь"
7. Для статистики (только админ): бот показывает количество жалоб и отзывов
