...
├── fix-database.sh         # Исправление структуры базы данных
├── qr_generator.html       # Генератор QR кода для бота
├── mysql/
│   ├── init/
│   │   ├── 01-init.sql     # Инициализация базы данных
│   │   └── 02-fix-tables.sql # Исправление структуры таблиц
...
2. Бот просит написать текст сообщения
3. Пользователь отправляет текстовое сообщение
4. Бот сохраняет данные в базу и отправляет email
...
```bash
# Если есть проблемы со структурой таблиц
./fix-database.sh
```
